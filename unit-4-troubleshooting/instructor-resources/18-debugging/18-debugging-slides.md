...
Up to now, you've (likely) been running into errors and (hopefully) googling the error message.

Let's start thinking about how we can find patterns in our error messages and build problem solving skills.
...
- Errors often say exactly what's wrong.
...
<iframe height="400px" width="100%" src="https://repl.it/repls/VibrantOutrageousMicroinstruction?lite=true" scrolling="no" frameborder="no" allowtransparency="true" allowfullscreen="true" sandbox="allow-forms allow-pointer-lock allow-popups allow-same-origin allow-scripts allow-modals"></iframe>
...
- One specific error (`except ValueError:`)
- Multiple specific errors (`except (ValueError, KeyError):`)
- Any error (`except:`)

Always try to specify the error, if possible! This way you know you're responding appropriately to the error that was thrown.
...
while is_hungry or fridge_has_food:
...
**Pro Tip**: If something is wonky and you don't know why, use the `print` statement.
